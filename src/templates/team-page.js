...
import AdvisorMenu from '../components/Team/AdvisorMenu';
...
export class TeamPageTemplate extends Component {
  render() {
    const {
      content, contentComponent, data, currSlug, isAdvisor,
    } = this.props;
    ...
    return (
      <div>
        <PageHelmet titleTemplate="%s | Team" title={name} description={about} />
        <TeamHeader isAdvisor={isAdvisor} />
        <Section background="rgba(20, 54, 76, 0.05)">
          <Container>
            <Row>
              <Column col="is-3" position="relative">
                <Box position="relative" height="100%">
                  {!isAdvisor && <TeamMenu currSlug={currSlug} />}
                  {isAdvisor && <AdvisorMenu currSlug={currSlug} />}
                  <Box
                    position="absolute"
                    height="100%"
